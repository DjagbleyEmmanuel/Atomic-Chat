...
function ThreadDetail() {
  ...
  // When true, the reasoning container keeps itself pinned to the bottom as new
  // tokens arrive. The user can opt out by scrolling up; scrolling back to the
  // bottom re-enables it. This prevents the auto-scroll from fighting the user
  // (jitter) while the model is still generating.
  const reasoningStickToBottom = useRef(true)

  const handleReasoningScroll = useCallback(() => {
    const el = reasoningContainerRef.current
    if (!el) return
    const distanceFromBottom = el.scrollHeight - el.scrollTop - el.clientHeight
    reasoningStickToBottom.current = distanceFromBottom < 4
  }, [])

  // Auto-scroll reasoning container to bottom during streaming, but only while
  // the user hasn't scrolled up. Once they scroll up to read, we leave them
  // alone until they return to the bottom.
  useEffect(() => {
    if (
      status === 'streaming' &&
      reasoningStickToBottom.current &&
