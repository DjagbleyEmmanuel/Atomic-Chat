...
const STREAMDOWN_PLUGINS = { code, mermaid, cjk } as any
  ...
  const mergedComponents = useMemo<Components | undefined>(() => {
    ...
    const CodeRenderer: Components['code'] = ({
      ...
      if (HTML_LANGUAGES.has(language)) {
        return <ArtifactTrigger code={codeText} streaming={!!isStreamingRef.current} />
      }

      if (language === 'mermaid') {
        const fence = makeFence(codeText)
        const reconstructed = `${fence}mermaid\n${codeText}\n${fence}`
        return <Streamdown {...delegateProps as any}>{reconstructed}</Streamdown>
      }

      return <CodeBlock code={codeText} language={language as BundledLanguage} />
    }
