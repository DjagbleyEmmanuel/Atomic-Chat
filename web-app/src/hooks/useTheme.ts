import type { AppTheme } from '@/types/appearance'
  ...
  persist(
    (set) => {
      ...
      const initialState = {
        ...
        setTheme: async (activeTheme: AppTheme) => {
          if (activeTheme === 'auto') {
            ...
            await getServiceHub()
              .theme()
              .setTheme(activeTheme === 'deep-dark' ? 'dark' : (activeTheme as ThemeMode))
            set(() => ({
              activeTheme,
              isDark: activeTheme === 'dark' || activeTheme === 'deep-dark'
            }))
          }
        },
      ...
      if (initialState.activeTheme === 'auto') {
        ...
        initialState.isDark =
          initialState.activeTheme === 'dark' ||
          initialState.activeTheme === 'deep-dark'
      }
